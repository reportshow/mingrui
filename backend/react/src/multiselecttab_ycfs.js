...
class MultiSelectYCFS extends React.Component {    
    render() {
	const {
	    autoWidth,
	    children,
	...
	<ul className="sys_spec_text" style={{marginLeft: marginleft}}>
		{
	      ...
	      <i onMouseEnter={()=>{$('#tipycfs').show();}}
		onMouseLeave={()=>{$('#tipycfs').hide();}}
		onTouchStart={()=>{$('#tipycfs').show();}}
...
export default MultiSelectYCFS;
